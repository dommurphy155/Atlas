...
def ensure_cli_symlink(spinner_live) -> None:
    dest = Path("/usr/local/bin/atlas2")
    if dest.exists() and not dest.is_symlink():
        spinner_live.update(_y(f"CLI symlink: {dest} exists, not ours"))
        return
    if dest.is_symlink():
        try:
            current = os.readlink(dest)
            if Path(current).resolve() == CLI_BIN_SRC.resolve():
                spinner_live.update(_g("CLI symlink: atlas2 already correct"))
                return
        except OSError:
            pass
        dest.unlink()
    try:
        ...
        spinner_live.update(_g(f"CLI symlink: {dest} → atlas/bin/atlas"))
    except PermissionError:
        spinner_live.update(_y(f"CLI symlink: couldn't write {dest}"))
