...
Replaces the previous bash-only installer with an interactive Python flow:
  ...
  1. Bootstrap proxy (venv, deps, dirs, systemd unit, CLI symlink)
  2. Detect OS
...
SERVICE_NAME = os.environ.get("ATLAS_SERVICE_NAME", "atlas-proxy-fork.service")
...
def ensure_systemd_unit(spinner_live) -> None:
    if shutil.which("systemctl") is None or os.geteuid() != 0:
        spinner_live.update(_c("[4/4] systemd skipped (no systemctl or not root)"))
        return
    ...
    if unit_path.exists():
        try:
            ...
            spinner_live.update(_y("[4/4] systemd unit exists, not writable"))
            return
        if "atlas_proxy setup wizard" not in content and "atlas_proxy/setup/install.sh" not in content:
            spinner_live.update(_y(f"[4/4] {unit_path.name} not ours — leaving alone"))
            return
    spinner_live.update(_c("[4/4] writing systemd unit..."))
    unit_text = (
        f"# Auto-generated by atlas_proxy setup wizard\n"
        f"[Unit]\nDescription=Atlas Translation Proxy\n"
        f"After=network-online.target\nWants=network-online.target\n\n"
        f"[Service]\nType=simple\nUser=root\n"
        f"WorkingDirectory={REPO_ROOT}\n"
        f"EnvironmentFile=-{REPO_ROOT}/.env\n"
        f"ExecStart={VENV_PY} -m proxy.main\n"
        f"Restart=on-failure\nRestartSec=5\n"
        f"StandardOutput=journal\nStandardError=journal\n"
        f"SyslogIdentifier=atlas-proxy\n\n"
    ...
    try:
        ...
        spinner_live.update(_g(f"[4/4] systemd unit enabled: {SERVICE_NAME}"))
    except PermissionError:
        spinner_live.update(_y("[4/4] couldn't write systemd unit (perms)"))
...
def ensure_cli_symlink(spinner_live) -> None:
    ...
    `atlas2` on this machine (per the project's local-command convention).
    Prefers system scope (`/usr/local/bin/atlas2`) when root/sudo is
    available AND the directory exists. Otherwise falls back to
    `~/.local/bin/atlas2` (XDG-conformant user-scoped location) and emits
    ...
    env = _rt.detect_env()
    sys_dest = Path("/usr/local/bin/atlas2")
    user_dest = Path(os.path.expanduser("~/.local/bin/atlas2"))
...
def ensure_runtime(spinner_live, dry_run: bool) -> str | None:
    """Detect the host environment, pick the best runtime, persist the choice.

    For systemd / systemd-user modes this also writes the unit file.
    Returns the chosen mode ("systemd" / "systemd-user" / "tmux" /
    "nohup" / "manual") or None on dry-run.

    The chosen mode is written to <repo>/data/runtime.json so subsequent
    commands (`atlas2 start` etc.) know what to drive.
    """
    import sys as _sys
    _sys.path.insert(0, str(REPO_ROOT / "atlas"))
    from bin import runtime as _rt
    ...
    # Show user what's happening — silently on the spinner, but capture for
    # the install summary.
    env_summary = (
        f"OS={env.os} root={env.is_root} sudo={env.sudo_works} "
        f"systemctl={env.has_systemctl} systemd-pid1={env.systemd_pid1} "
        f"systemd-user={env.has_user_systemd} tmux={'yes' if env.tmux_bin else 'no'} "
        f"nohup={env.has_nohup}"
    )

    if dry_run:
        mode = _rt.choose_mode(env)
        spinner_live.update(_c(f"[runtime] {mode} (DRY-RUN — would configure)"))
        CONSOLE.print(_dim(f"  env: {env_summary}"))
        return mode

    mode = _rt.choose_mode(env)
    spinner_live.update(_c(f"[runtime] chosen: {mode}"))
    CONSOLE.print(_dim(f"  env: {env_summary}"))

    # Systemd system: write unit if missing, daemon-reload, enable
    if mode == "systemd":
        unit_path = Path(f"/etc/systemd/system/{SERVICE_NAME}")
        if not unit_path.exists():
            ...
            try:
                ...
                subprocess.run(["systemctl", "daemon-reload"], check=False, capture_output=True)
                subprocess.run(["systemctl", "enable", SERVICE_NAME], check=False, capture_output=True)
                spinner_live.update(_g(f"  systemd unit written: {unit_path}"))
            except PermissionError:
                spinner_live.update(_y(f"  couldn't write {unit_path} (perm denied)"))

    # Systemd --user: write user unit (no sudo needed)
        ...
        if not unit_path.exists():
            unit_text = (
                f"# Auto-generated by atlas_proxy setup wizard\n"
                f"[Unit]\nDescription=Atlas Translation Proxy (user)\n"
                f"After=network-online.target\nWants=network-online.target\n\n"
                f"[Service]\nType=simple\n"
                f"WorkingDirectory={REPO_ROOT}\n"
                f"EnvironmentFile=-{REPO_ROOT}/.env\n"
        ...
        # Enable lingering so the user instance survives logout
        if shutil.which("loginctl"):
            subprocess.run(["loginctl", "enable-linger"], check=False, capture_output=True)
        subprocess.run(["systemctl", "--user", "enable", SERVICE_NAME], check=False, capture_output=True)
        spinner_live.update(_g(f"  systemd-user unit written: {unit_path}"))
    ...
    paths = {
        ...
        "log_file": str(REPO_ROOT / "data" / "run.log"),
        "pid_file": str(REPO_ROOT / "data" / "run.pid"),
    }
    rt_payload = {
        "mode": mode,
        "env": _rt.asdict(env),
    ...
    (REPO_ROOT / "data" / "runtime.json").write_text(
        __import__("json").dumps(rt_payload, indent=2)
    )
    spinner_live.update(_g(f"  runtime choice persisted to data/runtime.json"))
    return mode
...
def _proxy_is_up() -> bool:
    ...
    try:
        ...
        if shutil.which("systemctl") and os.geteuid() == 0:
            subprocess.run(["systemctl", "start", SERVICE_NAME], capture_output=True, timeout=15)
        else:
            subprocess.run(["bash", str(REPO_ROOT / "run.sh"), "--bg"], capture_output=True, timeout=15)
        time.sleep(2)
        try:
            with urllib.request.urlopen(f"{url}/health", timeout=2) as r:
                return r.status == 200
