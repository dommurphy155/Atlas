...
class OpenRouterClient:
    ...
    Non-streaming requests use a flat total timeout so a hung response fails
    fast and frees the key. Streaming requests need a different model: reasoning
    models sit silent for long stretches (prefill, thinking), so a per-read
    deadline that's too short kills healthy streams, while no cap at all lets a
    dead upstream hold a key forever. The stream client therefore uses a short
    connect timeout and a generous read timeout (the dead-stream backstop) with
    no total cap — the proxy bounds the *stream* lifetime via its keepalive
    wrapper, not httpx.
    """
    ...
    async def stream_chat(
        ...
        is the real upstream message (e.g. OpenRouter's "Validation: Temperature
        must be between 0 and 2") when ``status >= 400``, empty string otherwise
        — so the caller can surface it instead of the generic "upstream returned 400".
        ...
        # error (e.g. OpenRouter's 400 "Validation: Temperature must be between
                    ...
                    if chunk:
                        ...
                        if not _worker_limit_detected and on_worker_limit is not None:
                            try:
                                ...
                                if "worker" in chunk_str.lower() and "limit" in chunk_str.lower():
    ...
    def _headers(self, api_key: str) -> dict[str, str]:
        return {
            ...
            "HTTP-Referer": "https://github.com/atlas-proxy",
            "X-Title": "Atlas Proxy",
        }
...
def _extract_error_message(error_text: str) -> str:
    ...
    OpenRouter's chat-completions errors come in a few shapes:
    ...
    if isinstance(data, dict):
        # OpenRouter native: top-level "message".
        ...
        if isinstance(detail, str) and detail:
            return detail
        # OpenRouter 429 shape: {"status":429,"title":"Too Many Requests"}.
